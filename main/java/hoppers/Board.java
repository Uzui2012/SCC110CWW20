...
/**
 * Board class to represent and implement the Board of the game Hoppers. Only one Board should be created at a time.
 */
public class Board extends JFrame implements ActionListener
{
    /**
     * The 2D Square array that holds the squares representing a 5x5 Hoppers board.
     */
    ...
    /**
     * JFrame used on Board initialisation.
     */
    private JFrame frame;
    /**
     * JPanel used on Board initialisation.
     */
    private JPanel panel;
    /**
     * Boolean variable used to indicate if any square is selected at all
     */
    private boolean selected;
    /**
     * Number of green and red frogs currently on the Board
     */
    private int numGreen, numRed;
   
    /**
     * Creates a new Board object representing the passed selected level, used to be played Hoppers on. 
    ...
    public Board(int selectedLevel) {
        ...
        frame = new JFrame("Hoppers, level " + Integer.toString(selectedLevel)); //add level number
        frame.add(panel);
    ...
    /**
     * This method is called when the Java Virtual Machine recognises a JButton with an Action Listener has been clicked.
     * Performs the algorithm dictating what should occur when a JButton is clicked - in context.
     * Handles selection and deselection of a square, and the orcastrating of a legal move.
     * Then reloads and redraws the JFrame.
     */
    @Override
    public void actionPerformed(ActionEvent e) {
        ...
        if (selected) {
            ...
            if (calcLegalMove(x, y)) {
                ...
                selected = false;
                selectedSquare = new Square(99, 99, new JButton(), Piece.WATER);
                // Update the number of red and green frogs counted on screen
                updateNumFrogs();
                if (numLegalMoves() == 0 || numRed == 0) {
                    endGame();
                }
            } else {
    ...
    public void loadLevel(int selectedLevel){
        String fileName= "target/resources/levels.csv";
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int j = 0; j < 5; j++) {                
                if (level[j+k] .equals("w")) {
                    coordinates[i][j] = new Square(i, j, new JButton(new ImageIcon("target/resources/images/Water.png")), Piece.WATER);
                } else if (level[j+k].equals("p")) {
                    coordinates[i][j] = new Square(i, j, new JButton(new ImageIcon("target/resources/images/LilyPad.png")), Piece.PAD);
                } else if (level[j+k].equals("g")) {
                    coordinates[i][j] = new Square(i, j, new JButton(new ImageIcon("target/resources/images/GreenFrog.png")), Piece.GREEN);
                } else if (level[j+k].equals("r")) {
                    coordinates[i][j] = new Square(i, j, new JButton(new ImageIcon("target/resources/images/RedFrog.png")), Piece.RED);
                }
    ...
    public void removeFrog(int x, int y) {
        ...
        coordinates[q][r].getButton().setIcon(new ImageIcon("target/resources/images/LilyPad.png"));
    ...
    /**
     * This method updates the number of green and red frogs currently on the board, values of which must be updated for other methods in the class.
     */
    public void updateNumFrogs() {
        int green = 0;
        int red = 0;
        for (int i = 0; i < 5; i ++) {
            for (int j = 0; j < 5; j++) {
                if(coordinates[i][j].getPiece() == Piece.GREEN) {
                    green++;
                } else if (coordinates[i][j].getPiece() == Piece.RED) {
                    red++;
                }
            }
        }
    ...
    public void endGame() {
        if (numGreen == 0) {
            JOptionPane.showMessageDialog(frame, "Congratulations, you've completed this level", "End of Level", JOptionPane.PLAIN_MESSAGE);
